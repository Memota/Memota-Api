...
beforeEach(async () => {
  ...
  // Source: https://www.regextester.com/105777
  jwtRegEx = new RegExp(/^[A-Za-z0-9-_=]+\.[A-Za-z0-9-_=]+\.?[A-Za-z0-9-_.+/=]*$/)
...
describe("User controller", () => {
  ...
  it("loginUser -> status 200", async () => {
    ...
    const context = ({ status: undefined, body: undefined, request: { body: user } } as unknown) as Context
    await UserController.loginUser(context)
  ...
  it("sendPasswordReset -> status 200", async () => {
    ...
    await UserController.sendPasswordReset(context)
